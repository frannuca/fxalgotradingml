...
def train_portfolio_model(
    ...
    training period at once, compute the resulting return series, and take
    a gradient step on its (negated) Sharpe ratio.
    ...
    it. `weight_decay` is passed straight through to Adam as an L2 penalty.
    """
    ...
    for epoch in range(1, epochs + 1):
        ...
        weights = model(noisy_X_train)                                  # (n_train, n_assets)
        ...
        if epoch == 1 or epoch % max(epochs // 10, 1) == 0:
            logger.info("epoch %d/%d - train Sharpe %.4f", epoch, epochs, -loss.item())
...
class PortfolioResult:
    ...
    returns_train: np.ndarray   # realized daily portfolio log returns, in-sample
    returns_val: np.ndarray     # realized daily portfolio log returns, out-of-sample
    weights_train: np.ndarray   # (n_train, n_assets)
    weights_val: np.ndarray     # (n_val, n_assets)
...
def build_arg_parser(description: str) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Only used with --risk-overlay. Hard ceiling on the risk overlay's attenuation factor, "
             "in (0, 1]: even at full confidence it can never scale the position above this fraction",
    )
    parser.add_argument(
...
def _evaluate(model: nn.Module, data: _PreparedData, weight_scheme: str) -> PortfolioResult:
    """Run an already-trained-or-loaded model (eval mode, no grad) over both
    splits and package the result. Shared by the train path (after fitting)
    and the load path (skips fitting entirely).
    """
    model.eval()
    with torch.no_grad():
        weights_train = model(data.X_train).numpy()
        weights_val = model(data.X_val).numpy()
...
def _train_and_evaluate(data: _PreparedData, args: argparse.Namespace) -> PortfolioResult:
    ...
    train_portfolio_model(
        model, data.X_train, torch.tensor(data.next_returns_train_raw),
        epochs=args.epochs, lr=args.lr,
        weight_decay=args.weight_decay, noise_std=args.noise_std,
    )
    return _evaluate(model, data, args.weight_scheme)
...
def run_pipeline_multi_seed(args: argparse.Namespace) -> PortfolioResult:
    ...
    ensemble_result = _evaluate(ensemble_model, data, args.weight_scheme)
    logger.info(
...
def main() -> None:
    ...
    logger.info(
        "In-sample  (train) Sharpe %.3f | cumulative PnL %.4f",
        train_sharpe, float(result.returns_train.sum()),
    )
    logger.info(
        "Out-of-sample (val) Sharpe %.3f | cumulative PnL %.4f",
        val_sharpe, float(result.returns_val.sum()),
    )
